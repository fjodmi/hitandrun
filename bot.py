    ...
    with get_db() as conn:
        for p in tp:
            conn.execute("INSERT INTO training_players (training_id, player_id) VALUES (?, ?)",
                         (new_id, p[0]))
        conn.commit()
    # Deduct for all copied players
    for p in tp:
        add_payment(p[0], -t[2], "deduct", "тренировка " + new_date)
    return new_id
...
async def show_player_view(msg, player_id, edit=False):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🗑 Удалить участника", callback_data="player_delete:" + str(player_id))],
        back_kb("players")
    ])
    if edit:
        await msg.edit_text(text, reply_markup=kb, parse_mode="HTML")
...
async def cb_pay_add(callback: CallbackQuery, state: FSMContext):
    ...
    text = "Сколько пополнить для <b>" + p[1] + "</b> " + type_text + "?\n(например: 64 — за 4 тренировки)"
    await callback.message.edit_text(text, reply_markup=back_button("player_view:" + str(pid)), parse_mode="HTML")
...
async def show_training_view(msg, training_id, edit=False):
    ...
    text = ("📅 <b>Тренировка " + t[1] + "</b>\n" +
            "Участников: " + str(len(tp)) + "  |  Собрано: " + str(int(collected)) + " €\n\n")
...
async def process_duplicate_date(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        "✅ Тренировка скопирована на " + message.text.strip() + "!\n" +
        "Участники записаны, 16 € списано с каждого.",
        reply_markup=main_menu())
...
async def cb_training_add_player(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        "Выбери участника для " + t[1] + ":\n(16 € спишется с баланса)",
...
async def cb_training_remove_player(callback: CallbackQuery):
    ...
    buttons.append(back_kb("training_view:" + str(tid)))
    await callback.message.edit_text(
        "Кого убрать с тренировки?\n(16 € вернётся на баланс)",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def cb_training_remove_confirm(callback: CallbackQuery):
    ...
    p = get_player(pid)
    new_bal = p[2] + PRICE
    await callback.answer("↩️ " + p[1] + " убран. Возврат 16€. Баланс: " + str(round(new_bal, 0)) + "€")
    await show_training_view(callback.message, tid, edit=True)
